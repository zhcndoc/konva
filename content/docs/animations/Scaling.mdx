...
slug: Scaling.html
description: "了解如何使用带有 scaleX 和 scaleY 属性的 Konva.Animation 在 HTML5 canvas 上为形状缩放添加动画。"
---

要使用 Konva 为形状的缩放添加动画，我们可以通过
`Konva.Animation` 创建一个新动画，并定义一个函数，在每个动画帧中修改形状的缩放比例。
