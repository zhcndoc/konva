...
slug: Rotation.html
description: "学习如何使用 Konva.js 在 HTML5 Canvas 上实现形状旋转动画。使用 Konva.Animation 创建平滑的旋转动画。"
---

要使用 Konva 实现形状旋转动画，我们可以创建一个新的动画
`Konva.Animation`，并定义一个函数，在每一动画帧中修改形状的旋转。

在本教程中，我们将围绕左上角旋转一个蓝色矩形，
围绕其中心旋转一个黄色矩形，以及围绕外部点旋转一个红色矩形。
