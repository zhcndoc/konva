...
backgroundImage.src = 'https://new.konvajs.org/assets/beach.png';
...
function loadImages(sources, callback) {
  const assetDir = 'https://new.konvajs.org/assets/';
  const images = {};
  let loadedImages = 0;
  let numImages = 0;
...
const Animal = ({ name, startX, startY, outline, onScore }) => {
  ...
  const [image] = useImage(`https://new.konvajs.org/assets/${name}.png`);
  const [glowImage] = useImage(`https://new.konvajs.org/assets/${name}-glow.png`);
...
const AnimalOutline = ({ name, x, y }) => {
  const [image] = useImage(`https://new.konvajs.org/assets/${name}-black.png`);
  return image ? <Image image={image} x={x} y={y} /> : null;
};

const Background = () => {
  const [image] = useImage('https://new.konvajs.org/assets/beach.png');
...
const Animal = {
  ...
  setup(props, { emit }) {
    ...
    const [image] = useImage(`https://new.konvajs.org/assets/${props.name}.png`);
    const [glowImage] = useImage(`https://new.konvajs.org/assets/${props.name}-glow.png`);
...
const AnimalOutline = {
  ...
  props: ['name', 'x', 'y'],
  setup(props) {
    const [image] = useImage(`https://new.konvajs.org/assets/${props.name}-black.png`);
