...
description: "使用 Konva 在画布上对 300 个旋转矩形进行动画处理的压力测试，性能经过优化。"
---

此演示创建了 300 个具有随机大小、位置和颜色的矩形，然后通过旋转每个矩形来对它们进行动画处理。通过将图层的 `listening` 属性设置为 `false` 来优化动画性能，这样可以提高绘制性能，因为矩形将不会被绘制到命中图上。
...
**说明：** 此演示展示了 Konva 的动画能力，通过同时旋转 300 个矩形。观看这些形状在屏幕上平滑旋转。