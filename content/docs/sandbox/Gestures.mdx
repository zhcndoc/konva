...
slug: Gestures.html
description: "学习如何使用 Hammer.js 在 Konva 画布形状上处理滑动、捏合缩放、旋转和其他多点触控手势事件。"
---

## 如何监听画布形状上的滑动、捏合缩放、旋转和其他多点触控手势事件？

默认情况下 `Konva` 仅支持基本触摸事件，如 `touchstart`、`touchmove`、`touchend`。

你必须基于这些触摸事件手动实现手势事件。

如果你正在寻找整个舞台的平移和缩放逻辑，请查看 [多点触控缩放舞台演示](/docs/sandbox/Multi-touch_Scale_Stage.html)。

但我能够稍微修改 [Hammer.js](https://hammerjs.github.io/) 使其与 Konva 一起工作！

你可以在此处找到修改后的 [hammer.js 源代码](/js/hammer-konva.js)。

**说明：你可以在矩形上尝试不同的手势，如滑动、旋转、缩放、拖放、按压。对于桌面浏览器，你可以按住 `Shift` 键来模拟触摸事件。**
