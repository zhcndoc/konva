...
上述方法会将栅格图像放入 RGB 文档中。商业打印机通常要求 CMYK 分色、PDF/X 合规性、出血和裁切标记，而浏览器画布不会生成这些内容——这需要在浏览器外部执行渲染步骤。[Polotno](https://polotno.com/?utm_source=konvajs&utm_medium=docs&utm_content=canvas-to-pdf) 是一个基于 Konva 构建的商业设计编辑器 SDK，涵盖编辑器及其导出流程。
