---
title: 通过 JavaScript 构建可缩放和平移的无限画布
sidebar_label: 无限画布
hide_table_of_contents: true
slug: Infinite_Canvas.html
description: "使用 JavaScript 和 Konva.js 构建一个无限画布。滚动缩放，拖动平移，并带有网格背景。提供 React、Vue 和原生 JS 的交互式演示。"
---

无限画布让用户可以在无边界的工作区中自由平移和缩放——这是 Figma、Miro 和 tldraw 等工具的基础。本演示展示了如何使用 Konva 构建一个这样的画布：滚轮缩放并以光标位置为基准，拖动空白区域进行平移，拖动图形进行移动。

**说明：** 滚动进行缩放。拖动背景进行平移。拖动图形进行移动。
