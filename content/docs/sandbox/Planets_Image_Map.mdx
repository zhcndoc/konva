...
circlesLayer.batchDraw();
});

// load the image
Konva.Image.fromURL('https://new.konvajs.org/assets/planets.png', (planetsImage) => {
planetsLayer.add(planetsImage);
planetsLayer.batchDraw();
});
...
const App = () => {
  ...
  const [planetsImage] = useImage('https://new.konvajs.org/assets/planets.png');
