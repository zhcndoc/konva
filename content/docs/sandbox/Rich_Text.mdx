...
slug: Rich_Text.html
description: "使用 Konva 和 render-tag 在画布上显示带有粗体、斜体和颜色样式的丰富 HTML 文本。"
---
...
如果您想在画布上显示复杂样式，可以使用 [render-tag](https://polotno.com/render-tag/?utm_source=konvajs&utm_medium=docs&utm_content=rich-text)——这是一个使用 2D API 直接将 HTML + CSS 渲染到画布上的库。不使用 SVG，不使用 `foreignObject`，完全同步。
