...
description: "Konva 的 Bunnymark 性能压力测试，在 HTML5 画布上动画展示数百个跳跃的兔子精灵。"
---

## 跳跃兔子性能压力测试

此演示展示了同时移动许多 `Konva.Image` 对象时的性能。它改编自 [PixiJS 框架](https://pixijs.com/)的 [Bunnymark 演示](https://www.goodboydigital.com/pixijs/bunnymark/)。

注意：您可能会注意到，`Konva` 版本的性能比原始的 `PixiJS` 版本慢。这是因为 PixiJS 对 WebGL 渲染和此类动画进行了高度优化。而 Konva 仍在不断优化其内部实现，请记住此演示并不代表使用 Konva 构建的典型应用的性能。
...
const bunnyNodesMap = ref({}); // 存储 Konva 节点引用（索引对应）
