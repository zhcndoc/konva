...
description: "在 HTML5 canvas 上双击文本即可编辑，使用带有 Konva 变换支持的 textarea 覆盖层。"
---

由于 [许多原因](https://www.w3.org/TR/2dcontext/#best-practices)，用户无法直接编辑 `Konva.Text` 内容。事实上，canvas API 并非为此目的而设计。
可以在 canvas 上模拟文本编辑（通过绘制闪烁光标、模拟选区等）。
Konva 不支持这种情况。我们建议使用原生 DOM 元素（如 `input` 或 `textarea`）在 canvas 外部编辑用户输入。

如果你想启用完整的富文本编辑功能，请参阅 [富文本演示](/docs/sandbox/Rich_Text.html)。

**说明：双击文本进行编辑。输入内容。按 Enter 键或点击外部保存更改。**
