...
script.onload = () => {
  gifler('https://new.konvajs.org/assets/yoda.gif').frames(canvas, onDrawFrame);
};
...
const GifImage = () => {
  ...
  React.useEffect(() => {
    ...
    script.onload = () => {
      ...
      gifler('https://new.konvajs.org/assets/yoda.gif').frames(canvasRef.current, onDrawFrame);
    };
