---
title: 在 HTML5 Canvas 上使用自定义字体——加载 Web 字体并测量文本宽度
sidebar_label: 自定义字体
hide_table_of_contents: true
slug: Custom_Font.html
description: "使用 Konva 在 HTML5 canvas 上加载自定义 Web 字体。使用 Font Loading API 和缓存重置，修复字体加载前文本宽度测量错误的问题。"
---

## 如何在 html5 canvas 上绘制外部字体？

如果你想为 `Konva.Text` 使用自定义字体，只需要：
1. 将字体样式添加到页面
2. **在字体加载完成后**将 `fontFamily` 属性设置为所需的 font-face

但这里有一个重要的问题。当你为 DOM 元素（例如 `div` 或 `span`）设置字体时，浏览器会在字体加载完成后自动更新这些元素。但 canvas 文本的工作方式并不相同。你需要再次重绘 canvas。

**注意：** 对于不支持原生 Font Loading API 的旧版浏览器，你可以使用宽度测量方法——先使用备用字体测量文本宽度，然后定期检查自定义字体的宽度是否发生变化（这表示字体已加载）。

### 加载字体

你可以使用以下异步函数，它将原生 Font Loading API 与可靠的宽度测量回退方案和时序保护结合在一起：
