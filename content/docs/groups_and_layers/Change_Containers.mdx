...
description: "了解如何使用 moveTo() 方法在 Konva 中的组、图层和容器之间移动形状。"
---

要使用 Konva 将形状从一个容器移动到另一个容器，我们可以使用 `moveTo()` 方法，该方法需要一个容器作为参数。
容器可以是另一个舞台、图层或组。你也可以将组移动到其他组和图层中，或将组中的形状直接移动到其他图层中。

**说明：拖放这些组，并观察红色矩形绑定到黄色组还是蓝色组。使用左侧的按钮将方框从一个组移动到另一个组。**
