...
description: "通过 Konva.js 形状缓存加速 HTML5 Canvas 渲染。将复杂形状缓存为图像以显著提高性能。"
---
...
1. 不要为没有过滤器的简单形状缓存。直接渲染它可能比从缓存版本渲染更快。
2. 每个已缓存的节点都会创建多个画布缓冲区。因此不要过度使用，因为它会消耗大量内存。
3. 缓存形状组通常比单独缓存每个形状更好。
4. 记得始终在有缓存和无缓存的情况下测量性能，以查看实际差异。
...
stage.on('click', () => {
  addCircles(1000);
  countText.text('圆圈：' + group.children.length);
});
