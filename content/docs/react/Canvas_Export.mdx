...
description: "学习如何在 React 中使用 react-konva 将画布导出为图像。使用 stage.toDataURL() 将画布保存为 PNG 或 JPEG。"
---
...
浏览器会限制画布的宽度、高度和总面积，因此较高的 `pixelRatio` 可能会返回空白图像，而不是错误。有关限制以及解决这些限制的方法，请参阅[高质量导出](/docs/data_and_serialization/High-Quality-Export.html)。
