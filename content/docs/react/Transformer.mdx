...
description: "学习如何在 React 中使用 react-konva Transformer 调整画布形状的大小和旋转。为形状添加交互式选择手柄。"
---
...
`Transformer` 绘制控制点并应用缩放。吸附到其他对象、对齐辅助线、多选共享边界框以及每个形状的纵横比规则都需要由你自行构建——请参阅[对象吸附](/docs/sandbox/Objects_Snapping.html)了解一种实现方式。
