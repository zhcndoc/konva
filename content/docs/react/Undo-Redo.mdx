...
slug: Undo-Redo.html
description: "学习如何使用 React 状态历史管理来实现画布交互的撤销和重做功能。"
---
...
上述历史记录每个步骤只记录一个值。生产级编辑器必须记录分组操作，以便多选拖动可以作为一个步骤撤销，还要记录操作完成后才加载完成的变换和图像。这种状态机通常会比绘图代码更庞大，因此应围绕文档操作而不是原始节点状态来规划历史记录。
