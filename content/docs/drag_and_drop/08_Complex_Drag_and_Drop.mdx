...
description: "了解如何使用 dragmove 事件在 Konva 中将拖放移动限制在自定义边界和区域内。"
---

要在 Konva 中将拖放节点的移动限制在区域内，我们可以使用 `dragmove` 事件来定义节点无法跨越的边界。

_提示：你可以使用 `shape.absolutePosition()` 方法获取或设置节点的绝对位置，而不是使用相对位置 `x` 和 `y`。_

**说明：** 拖放浅蓝色矩形，观察它被限制在 y = 50 的假想边界下方。拖放黄色矩形，观察它被限制在假想圆形内部。
