...
description: "学习如何使用 dragmove 事件在 Konva 中将拖放运动限制在自定义边界和区域内。"
---

要在 Konva 中将拖放节点的运动限制在区域内，我们可以使用 `dragmove` 事件来定义节点无法跨越的边界。

_提示：你可以使用 `shape.absolutePosition()` 方法来获取/设置节点的绝对位置，而不是相对的 `x` 和 `y`。_

**说明：** 拖放浅蓝色矩形，观察它被限制在 y = 50 的假想边界下方。拖放黄色矩形，观察它被限制在一个假想圆圈内。
