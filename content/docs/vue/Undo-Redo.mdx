...
slug: Undo-Redo.html
description: "了解如何通过 Vue 响应式系统跟踪状态历史，在 Vue 中实现画布形状的撤销和重做功能。"
---
...
请注意，我们使用 Vue 的响应式系统来管理状态，但通过将历史记录保存在 `ref` 中，谨慎地避免了不必要的重新渲染。

## 手动构建的历史记录何时不再适用

上述历史记录每个步骤只记录一个值。生产级编辑器必须记录成组的操作，以便多选拖拽可以作为一个步骤撤销，还要处理操作完成后才加载完毕的变换和图像。这种状态机通常会比绘图代码更庞大，因此应围绕文档操作，而不是原始节点状态来规划历史记录。
