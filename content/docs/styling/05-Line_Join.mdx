...
slug: Line_Join.html
description: "学习如何使用 Konva.js 在 HTML5 Canvas 上为形状设置线条连接样式（miter, bevel, round）。"
---

要使用 Konva 为形状设置线条连接，我们可以在实例化形状时设置 `lineJoin` 属性，或者使用 `lineJoin()` 方法。

`lineJoin` 属性可以设置为 `miter`、`bevel` 或 `round`。除非另有指定，默认的线条连接是 `miter`。

**说明：** 鼠标悬停在三角形上以更改线条连接样式。
