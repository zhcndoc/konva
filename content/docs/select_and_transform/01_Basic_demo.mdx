...
description: "学习如何使用 Konva.js Transformer 在 HTML5 Canvas 上选择、调整大小和旋转形状。为调整大小和旋转任何形状添加交互式手柄。"
---
...
_注意:_ 变换工具在调整节点大小时不会更改节点的 `width` 和 `height` 属性。相反，它会更改 `scaleX` 和 `scaleY` 属性。
