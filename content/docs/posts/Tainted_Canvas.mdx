...
description: "修复在 Konva 中导出或过滤跨域图像时出现的 'Tainted canvases may not be exported' CORS 错误。"
---
...
这是一个 [CORS 错误](https://developer.mozilla.org/en-US/docs/Web/HTML/How_to/CORS_enabled_image)。出于安全原因，当你从其他域加载图像时，浏览器可能会将画布标记为已污染。在这种情况下，浏览器会阻止将画布导出为 `dataURL` 或 `imageData`（这正是我们在导出或使用过滤器时所做的操作）。
...
**在所有情况下它可能仍然无法正常工作。如果它不起作用，那么你必须以不同的方式配置你的服务器（这超出了 Konva 的范围），或者你可以尝试将图像存储在其他支持 CORS 请求的位置。**
