...
description: "学习如何使用常见的缓动函数（如 EaseIn、EaseOut 和 EaseInOut）来实现平滑的 Konva.js 补间动画。"
---

要在 Konva 中创建非线性缓动补间，我们可以将 `easing` 属性设置为缓动函数。除了 `Konva.Easings.Linear` 之外，最常见的缓动还有：

- `Konva.Easings.EaseIn`
- `Konva.Easings.EaseInOut`
- `Konva.Easings.EaseOut`

查看所有可用缓动，请前往 [缓动文档](/api/Konva.Easings.html)。

**说明：将鼠标悬停或触摸启动方块，以不同的缓动函数对其进行补间动画。**
