...
description: "了解如何使用 EaseIn、EaseOut 和 EaseInOut 等常见缓动函数，实现流畅的 Konva.js 补间动画。"
---

要使用 Konva 创建非线性缓动补间动画，我们可以将 `easing` 属性设置为缓动函数。除 `Konva.Easings.Linear` 外，其他最常见的缓动函数包括：

- `Konva.Easings.EaseIn`
- `Konva.Easings.EaseInOut`
- `Konva.Easings.EaseOut`

如需查看所有可用的缓动函数，请前往[缓动文档](/api/Konva.Easings.html)。

**说明：对方框执行 mouseover 或 touchstart，以使用不同的缓动函数对它们进行补间动画。**
