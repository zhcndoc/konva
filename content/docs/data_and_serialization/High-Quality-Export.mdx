...
description: "学习如何使用 Konva.js 将 HTML5 Canvas 导出为高质量 PNG 或 JPEG 图像。使用 stage.toDataURL() 配合 pixelRatio 进行视网膜级质量的导出。"
---
...
每个浏览器都会限制 canvas 的宽度、高度和总面积，并且移动设备上的限制更低。超过该限制后，较大的 `pixelRatio` 会静默失败：调用会返回空白或被截断的图像，而不是抛出错误。大尺寸输出必须进行平铺，或在浏览器之外进行渲染。
