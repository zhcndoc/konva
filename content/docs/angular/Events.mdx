...
slug: Events.html
description: "学习如何在 Angular 中使用 ng2-konva 事件绑定来处理画布形状上的鼠标、触摸和指针事件。"
---

要在 Angular 中处理指针事件，请将 `(mousemove)` 和 `(mouseout)` 等监听器直接附加到 `ng2-konva` 组件上。

此演示在鼠标移过三角形时，根据舞台指针坐标更新文本标签。

**说明**：将鼠标移过三角形以更新坐标标签，然后移出以重置文本。
