...
slug: index.html
description: "开始使用 ng2-konva，它是 Konva.js 的官方 Angular 绑定。绘制形状、处理事件，并使用 Angular 指令构建交互式 Canvas 应用程序。"
---

## 如何在 Angular 中使用 Canvas？

`ng2-konva` 是一个使用 [Angular](https://angular.dev/) 绘制复杂 Canvas 图形的 JavaScript 库。它为 [Konva Framework](https://konvajs.org/) 提供声明式和响应式绑定。

[Github 仓库](https://github.com/konvajs/ng2-konva)

它旨在让 [Angular](https://angular.dev/) 与 HTML5 Canvas 库协同工作。目标是拥有与普通 Angular 类似的声明式标记，以及类似的数据流模型。
...
对于 Angular 21 应用，简单的静态配置适用于简单的演示。如果从异步回调（例如图像加载）更新配置，建议使用 Angular 信号。
...
如需查看完整的属性和方法列表，请参阅 [Konva API 参考](/api/Konva.html)。
