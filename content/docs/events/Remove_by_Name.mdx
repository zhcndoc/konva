...
description: "学习如何在 Konva 中使用 on() 和 off() 方法配合命名事件，通过命名空间移除特定的事件监听器。"
---

Konva 事件命名空间用于在命令式代码中标识相关的监听器。在事件类型后添加命名空间，例如 `click.menu`。然后将相同的名称传递给 `off()`，以移除该监听器。

**说明：**选择圆形以运行两个监听器。使用每个按钮移除一个监听器。然后再次选择圆形。

`react-konva` 不会通过 React 事件属性公开 Konva 事件命名空间。React 为每种事件类型提供一个属性，例如 `onClick`。在 React 中存储启用状态，并有条件地将一个分发器传递给该属性。
